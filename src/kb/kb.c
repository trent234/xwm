...
void
updatekeys() {
    int i = 0, row_start, x, y = 0, h, base, num_keys;
    int total_rows = 1;

    // Count the number of rows by counting row separators (each key with keysym == 0) and add one for the final row
    for (i = 0; i < LENGTH(keys); i++) {
        if (keys[i].keysym == 0)
            total_rows++;
    }
    ...
    while (i < LENGTH(keys)) {
        ...
        row_start = i;
        base = 0;
        // Calculate total width factor for this row.
        while (i < LENGTH(keys) && keys[i].keysym != 0) {
            base += keys[i].width;
            i++;
        }
        num_keys = i - row_start;
        int row_h = (row < total_rows) ? h : (wh - y);
        if (num_keys > 0) {
            x = 0;
            for (int j = row_start; j < i; j++) {
                keys[j].x = x;
                keys[j].y = y;
                keys[j].w = keys[j].width * (ww - 1) / base;
                keys[j].h = row_h;
                x += keys[j].w;
            }
        }
        // Skip the row separator if present.
        if (i < LENGTH(keys) && keys[i].keysym == 0)
            i++;
        y += row_h;
    }
}
